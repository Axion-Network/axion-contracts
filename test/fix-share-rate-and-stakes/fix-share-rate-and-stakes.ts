...
describe('Fix Share Rates & Stakes', () => {
  ...
  it('should fix a v1 stake that has been withdrawn', async () => {
    ...
    const sessionDataV1 = await stakingV1.sessionDataOf(staker.address, sessionId);
  ...
  it('should not fix a stake which has already been fixed', async () => {
    ...
    await expect(staking.connect(setter).fixV1Stake(staker.address, sessionId)).to.be.revertedWith("Stake already fixed and or withdrawn");
  })
  ...
  it('should not fix a stake which has not been unstaked', async () => {
    ...
    await expect(staking.connect(setter).fixV1Stake(staker.address, sessionId)).to.be.revertedWith("Stake has not been withdrawn");
  })
  it('should not fix a stake which was staked AFTER last session id', async () => {
    ...
    await expect(staking.connect(setter).fixV1Stake(staker.address, sessionId)).to.be.revertedWith("Invalid sessionId");
  })
});
